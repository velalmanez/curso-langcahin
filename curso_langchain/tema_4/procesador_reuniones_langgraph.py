...
# declaracion de modelo a utilizar
...
def extract_participants(state: State) -> State:
    ...
    Responde UNICAMENTE con la lista de nombres separados por comas, sin explicaciones adicionales.
    Ejemplo: Juan Garcia, Maria Lopez, Carlos Ruiz
    """
...
def identify_topics(state: State) -> State:
    prompt = f"""
    Identifica los 3-5 temas principales discutidos en esta reunion.
    ...
    response = llm.invoke(prompt)
    topics = [t.strip() for t in response.cotent.split(';') if t.strip()]
...
def generate_minutes(state: State) -> State:
    ...
    # recupera las listas de elementos del state y les da un formato en string
    ...
    # utiliza la informacion del state para generar la minuta
    prompt = f"""
...
def create_workflow():
    ...
    # confiugracion de flujo secuencial 
...
def transcribe_media_direct(file_path: str)-> str:
    try: 
        print("Transcribiendo de API Google ...")
